<x-app-layout>
    <div class="container mx-auto px-4 py-8">
        ...
        <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-lg shadow-md max-w-2xl mx-auto space-y-4">
            @csrf
            @method('PUT')
                ...
                <div class="mt-2 flex justify-center rounded-lg border border-dashed border-gray-900/25 px-6 py-10">
                    <div class="text-center">
                        @if ($product->image_url)
                            <img src="{{ asset($product->image_url) }}" alt="{{ $product->name }}" class="mx-auto h-48 w-48 object-cover rounded-lg">
                        @else
                        ...
                        <div class="mt-4 flex text-sm text-gray-600 justify-center">
                            <label for="image_url" class="relative cursor-pointer rounded-md bg-white font-semibold text-indigo-600 focus-within:outline-none focus-within:ring-2 focus-within:ring-indigo-600 focus-within:ring-offset-2 hover:text-indigo-500">
            ...
            <div>
                ...
                <select id="is_prescription" name="is_prescription" required
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring focus:ring-blue-300">
                    <option value="0" {{ old('is_prescription', $product->is_prescription) == 0 ? 'selected' : '' }}>No</option>
                    <option value="1" {{ old('is_prescription', $product->is_prescription) == 1 ? 'selected' : '' }}>Sí</option>
                </select>
            </div>
            ...
            <div>
                ...
                <input type="date" id="expiration_date" name="expiration_date" value="{{ old('expiration_date', $product->expiration_date) }}"
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring focus:ring-blue-300">
            </div>
            ...
            <div>
                ...
                <input type="text" id="manufacturer" name="manufacturer" value="{{ old('manufacturer', $product->manufacturer) }}" required
                    class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring focus:ring-blue-300">
            </div>
