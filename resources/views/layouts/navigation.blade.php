<div class="w-full bg-white shadow-lg border-b border-gray-200">
    <div class="max-w-screen-xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <div class="hidden sm:flex sm:items-center space-x-8">
            <a href="{{ route('home') }}"
                class="text-gray-700 hover:text-green-600 text-base font-medium transition duration-300">
                Home
            </a>
            <a href="{{ route('about') }}"
                class="text-gray-700 hover:text-green-600 text-base font-medium transition duration-300">
            ...
            @if (session('error'))
                <div id="alert-message"
                    class="flex items-center bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded shadow-lg animate-slide-down mt-4"
                    role="alert">
            ...
            <form action="{{ route('products.search') }}" method="GET" class="flex items-center">
                <div class="relative flex items-center border border-gray-300 rounded-full bg-white shadow focus-within:border-green-500 focus-within:ring-1 focus-within:ring-green-300"
                    style="width: 300px;">
            ...
            @auth
                <div class="relative">
                    <button id="admin-menu-button"
                        class="bg-gray-200 text-gray-700 px-4 py-2 rounded-lg font-medium hover:bg-gray-300 transition duration-300">
                        {{ Auth::user()->name }}
                    </button>
    ...
    <div id="mobile-menu" class="hidden px-6 py-4 space-y-2 bg-white border-t border-gray-200 sm:hidden">
        <a href="{{ route('home') }}"
            class="block text-gray-700 hover:text-green-600 text-lg font-medium transition duration-300">
            Home
        </a>
        ...
        @auth
            ...
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit"
                    class="block w-full text-left text-gray-700 hover:text-green-600 text-lg font-medium transition duration-300">
                    Cerrar sesión
                </button>
            </form>
        @else
